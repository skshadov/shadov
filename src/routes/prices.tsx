...
const DESCRIPTION =
  "Полный прайс без «от»: работа, материал и цена под ключ по каждой позиции. Механизированная штукатурка, мокрая стяжка, тёплый пол, разводка электрики и сантехники в Москве и области.";
...
function PricesPage() {
  return (
    <InfoPageLayout
      ...
      h1="Цены на работы"
      intro={`Показываем сразу три цифры по каждой позиции: работа, материал и итог под ключ. Все надбавки — за слой, за объём, за нестандарт — перечислены отдельными строками, поэтому итог в смете совпадает с прайсом. Цены актуальны на ${PRICES_ACTUAL_DATE.toLowerCase()} для Москвы и области до 30 км от МКАД.`}
    >
      <section aria-label="Краткая сводка цен" className="overflow-hidden rounded-xl border border-border bg-card">
        <header className="border-b border-border px-5 py-4 md:px-6">
          <h2 className="font-display text-lg font-semibold md:text-xl">Коротко: сколько стоит под ключ</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Базовая цена при стандартных условиях. Полные условия — в таблицах ниже.
          </p>
        </header>
                ...
                <article key={p.title} className="rounded-xl border border-border bg-card p-5">
                  ...
                  <dl className="mt-4 space-y-1 border-t border-border pt-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <dd className="font-medium">{formatRub(p.workTotal)}</dd>
                    </div>
                    <div className="flex justify-between">
                      <dt className="text-muted-foreground">Под ключ с материалом</dt>
                      <dd className="font-semibold text-primary">{formatRub(p.turnkeyTotal)}</dd>
                    </div>
                    <div className="flex justify-between">
      ...
      <InfoSection title="Что влияет на итоговую сумму">
        <p>
          ...
          надбавки до подписания сметы, и после этого сумма не растёт.
        </p>
        <p>
