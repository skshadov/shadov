...
function getTrustedClientKey(req: Request, fallback: string): string {
  // Cloudflare переписывает cf-connecting-ip на каждом запросе — подделка
  // через клиентский заголовок невозможна (подтверждено debug-эндпоинтом).
  const cf = req.headers.get("cf-connecting-ip");
  if (cf && /^[0-9a-fA-F.:]+$/.test(cf) && cf.length <= 64) return cf;
...
export function createHandler(config: HandlerConfig) {
  ...
  return async (req: Request): Promise<Response> => {
    ...
    if (RATE_LIMIT_SALT.length < SALT_MIN_LENGTH) return reject(500, "server_not_configured", originAllowed);
    ...
    const rlKeyMaterial = getTrustedClientKey(req, body.submission_id);
    const keyHash = await sha256(`${rlKeyMaterial}|${RATE_LIMIT_SALT}`);
