...
// Stage 3B: бизнес-логика edge function в _shared/handler.ts; prod и test
// варианты — тонкие обёртки.
...
t("test fn использует общий handler с testMode:true", /testMode:\s*true/.test(testFn));
...
t("edge: RATE_LIMIT_SALT min length 32", /SALT_MIN_LENGTH\s*=\s*32/.test(edge) && /RATE_LIMIT_SALT\.length\s*<\s*SALT_MIN_LENGTH/.test(edge));
