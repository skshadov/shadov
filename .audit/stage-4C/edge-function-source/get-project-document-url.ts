/**
 * Stage 4 — выдача короткоживущего signed URL для документа проекта.
 */
...
Deno.serve(async (req: Request) => {
  ...
  const { data: doc } = await userClient
    ...
    .select("id,storage_path,is_visible_to_client")
    .eq("id", documentId)
  ...
  const { data: signed, error: signErr } = await admin.storage
    .from("project-documents").createSignedUrl(doc.storage_path, SIGNED_URL_TTL);
